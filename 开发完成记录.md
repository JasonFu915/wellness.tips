...
## 日期：2026-02-13 (SEO 进阶与功能完善)

### 1. 自动化脚本升级
- **智能配图逻辑增强** (`generate_daily_tip.py`):
  - 同步了 `migrate_covers.py` 的**三级搜索策略**（全标题 -> 前3词 -> 通用关键词）。
  - 显著降低了因长标题导致 Unsplash 搜索无结果的概率，确保新生成文章配图率接近 100%。

### 2. 网站功能与体验优化
- **标签聚合系统 (Tag System)**:
  - 新增 **标签归档页** (`app/[lang]/tags/[tag]/page.js`)，汇集同一标签下的所有文章。
  - 更新 **Sitemap** (`app/sitemap.js`)，自动收录所有标签页面，增加搜索引擎入口。
  - 文章详情页标签改为**内部链接**，构建更紧密的内链网络。
- **相关文章推荐 (Related Posts)**:
  - 开发 `getRelatedPosts` 算法 (`lib/posts.js`)，基于标签匹配度推荐文章。
  - 在文章侧边栏展示相关推荐，替代原有的“最新文章”，降低跳出率。
- **UI/UX 修复**:
  - 修复首页 Hero 区域 `overflow-hidden` 导致搜索下拉框被遮挡的问题。
  - 优化图片加载 (`next/image`)，移除 `unoptimized` 属性，配置 `images.unsplash.com` 域名白名单，启用 WebP 自动转换与响应式加载。

### 3. 高级 SEO 结构化数据
- **FAQ Schema 自动化**:
  - 开发 Markdown 解析器 (`lib/markdown.js`)，自动提取文章末尾的 FAQ 章节。
  - 在详情页自动注入 `FAQPage` JSON-LD 数据，争取 Google 搜索结果的问答展示位。
