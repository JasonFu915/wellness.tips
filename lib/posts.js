...
export function getRelatedPosts(lang, currentSlug, currentTags = [], limit = 5) {
  const allPosts = getAllPosts(lang);
  const normalizedCurrentSlug = normalizeSlug(currentSlug);
  
  // Filter out current post
  const candidates = allPosts.filter(post => post.slug !== normalizedCurrentSlug);
  
  if (!currentTags || currentTags.length === 0) {
    // If no tags, just return most recent
  ...
  scored.sort((a, b) => {
    ...
    return parseDate(b.post.publishDate) - parseDate(a.post.publishDate);
  });
...
export function getPostsByTag(lang, tag) {
  ...
  return all.filter(post => 
    post.tags && post.tags.some(t => t.toLowerCase() === normalizedTag)
  );
}
